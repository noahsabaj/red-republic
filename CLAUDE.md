...
1. **Nothing the simulation knows is invisible.** Every new simulation fact ships with its UI in the same commit, and is controllable wherever it is a decision rather than a consequence. `ExposureTests` enforces it. There used to be two guards, one per boundary, because a fact could reach the shell and stop there — there is no middle layer any more, so "is this exposed" and "does a screen reach it" are one question. It checks the **nouns** — what a republic is — and the **verbs**, because for the life of the port it checked only the first and thirteen of the twenty-three things a player can ask for reached no control at all. Each half carries a work list (`NotYetReached`, `NoControlYet`) of what is still waiting for a screen; wiring one up fails the build until its line goes. **It is a source scan and has a known blind spot**: a scan finds its name in a file that does not compile, which is what CI loading the game covers. The scan is receiver-qualified — it looks for `world.X` rather than `.X` — because the loose form was permanently satisfied for every fact whose name is an ordinary word.
...
- **Systems propose, one writer applies, and each declares what it may change.** No generic `{field, value}` hatch: that is "write anything" with extra steps and leaves a guard nothing to check. Declarations are checked in **both** directions — the half that rots is a write-set quietly claiming more than its system emits, and it went unchecked for the whole port. The second direction needs a republic driven into every corner it has, because a declaration is only claiming too much if *nothing the republic can do* reaches it; `Busy` is that republic and it is a fixture rather than a game.
...
- **Balance is authored in `game/data/manifest.json`.** It is the data, not a generated copy of it: the simulation reads it and nothing writes it but a person. Its checksum is what proves the file the game loaded is the file somebody meant to write, byte for byte — the shape of the table as well as its figures, so a resource that changed form or a bill that swapped brick for steel at the same tonnage is a different table. `--stamp-balance` re-stamps it after a deliberate edit, and a save carries the checksum it was played against.

## Commands

- `dotnet test src/RedRepublic.Sim.Tests` — the gate. Runs in half a minute and needs no engine. Most of that is two tests that play a republic rather than checking a rule: `No_system_claims_more_than_it_changes` drives one into every corner it has, and `DirectorTests` plays the opening for two years and asserts something happened.
...
- **`pwsh tools/package.ps1`** — exports the playable game into `build/` and compiles the Windows installer into `dist/`. `-NoInstaller` stops after the export, which is the loop for testing a change; `build/RedRepublic.exe` beside its data folder is the layout every other Godot project on this machine uses. It also stamps `game/export_presets.cfg` from the project's `<Version>` and stops if that changed anything, because the exported executable's version resource comes from the preset and .NET never sees it. CI runs it with `-NoInstaller` on a Windows runner, which is the only job on the platform the product ships to.
