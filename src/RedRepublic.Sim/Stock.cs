...
/// <remarks>
/// <para>
/// Every building owns a run of <c>Resources.Length</c> slots inside a single
/// array, so building <c>b</c>'s holding of resource <c>r</c> lives at
...
/// </para>
/// </remarks>
public sealed class Stock(int resources, int buildings)
{
    private readonly double[] _t = new double[resources * buildings];

    public int Resources { get; } = resources;

    public int Buildings { get; } = buildings;

    /// <summary>The whole table, for a caller that wants to read it in bulk.</summary>
    public ReadOnlySpan<double> All => _t;
