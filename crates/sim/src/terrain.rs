...
//! # Cell size
//!
//! [`CELL_SIZE`] is 10 m: fine enough that a detached house and a shop are
//! distinguishable footprints rather than the same rounded square, and coarse
//! enough that a 10 km republic is a million cells rather than a hundred
//! million. Like the tick, it is a **starting value chosen to be changed** —
//! everything is written against the constant so re-measuring is a one-line
//! experiment.
//!
...
/// The side length of one terrain sample cell. See the module docs.
pub const CELL_SIZE: Metres = Metres(10.0);
...
impl Terrain {
    /// A flat grass square `extent` metres on a side — the test fixture, and
    /// the base a generator builds on.
    pub fn flat(extent: Metres) -> Self {
        let cells = Self::cells_for(extent);
    ...
    fn cells_for(extent: Metres) -> u32 {
    ...
    pub fn extent(&self) -> Metres {
        Metres(f64::from(self.cells) * CELL_SIZE.0)
    }
    ...
    fn index_of(&self, p: Point) -> Option<usize> {
        ...
        let cx = (p.x.0 / CELL_SIZE.0) as u32;
        let cy = (p.y.0 / CELL_SIZE.0) as u32;
    ...
    pub fn cell_centre(&self, cx: u32, cy: u32) -> Point {
        Point::new(
            Metres((f64::from(cx) + 0.5) * CELL_SIZE.0),
            Metres((f64::from(cy) + 0.5) * CELL_SIZE.0),
        )
    }
    ...
    fn every_cell_in(
        ...
        let cell_min_x = (min_x / CELL_SIZE.0) as u32;
        let cell_max_x = ((max_x / CELL_SIZE.0).ceil() as u32).min(self.cells);
        let cell_min_y = (min_y / CELL_SIZE.0) as u32;
        let cell_max_y = ((max_y / CELL_SIZE.0).ceil() as u32).min(self.cells);
...
pub fn generate_terrain(seed: u64, extent: Metres, plan: &TerrainPlan) -> Terrain {
    let mut terrain = Terrain::flat(extent);
