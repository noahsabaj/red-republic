...
- Every bug fix lands with a regression test in `src/game/__tests__/`. Engine tests build worlds with `helpers.ts` (`makeEngine()` = flat map, no starting base, **no border**, **calm weather** — pass `weather:` to script conditions; `flatBorderMap()` opts into border rules; real-map behavior needs `new GameEngine` directly). Remember `pop` clamps to housing capacity at day end — place beds before setting `pop`.
